...
export default function WhatsAppButton() {
  ...
  const phone = "5215548792525";
  const message = isClientAccount
    ? "Hola, coach. Tengo una duda con la técnica de un ejercicio de mi rutina. ¿Me puedes orientar?"
    : "Hola! Vi tu página y me gustaría recibir información sobre los planes de entrenamiento.";
  const url = `https://wa.me/${phone}?text=${encodeURIComponent(message)}`;
  const label = isClientAccount
  ...
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className={`fixed bottom-6 left-6 z-50 flex items-center rounded-full shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#25d366] ${
        isClientAccount
          ? "h-12 w-12 justify-center"
          : "gap-2.5 py-3 pl-3 pr-5"
      }`}
      style={{
        background: "#25d366",
